...
func createTable() {
	...
	for _, platform := range array {
		query := fmt.Sprintf("CREATE TABLE IF NOT EXISTS " + platform + "_pipelines(pipelineid INT PRIMARY KEY, sha VARCHAR, ref VARCHAR, status VARCHAR, web_url VARCHAR, release_tag VARCHAR, coverage VARCHAR, total_coverage_count VARCHAR, valid_test_count VARCHAR, kubernetes_version VARCHAR);")
